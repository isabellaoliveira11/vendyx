...
function Home() {
  ...
  const fetchSalesAndProducts = async () => {
    ...
    try {
      ...
      const salesToday = validSales.filter((sale: Sale) => format(new Date(sale.createdAt), 'yyyy-MM-dd') === todayFormatted);
      const totalSalesValueToday = salesToday.reduce((acc, sale) => acc + sale.total, 0);
      setTodaySalesValue(totalSalesValueToday);

      const salesThisMonth = validSales.filter((sale: Sale) => isSameMonth(new Date(sale.createdAt), today));
      const totalSalesValueThisMonth = salesThisMonth.reduce((acc, sale) => acc + sale.total, 0);
      setTotalMonthSalesValue(totalSalesValueThisMonth);

      setSales(validSales.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 5));
